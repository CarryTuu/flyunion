				...
				<div class="row clearfix">
					...
					<div class="col-md-6">
						<el-card class="box-card-large">
							<div slot="header">
								<span class="text-secondary">可执行航线{{ flightPlan.flightCode }}的基本信息</span>
							</div>
							<br>
							<br>
							<el-form v-model="flightPlan">
								...
								<br>
								<el-form-item>
									<el-button type="primary" plain style="margin-left: 300px" @click="updateFlightPlan()">修改航线信息</el-button>
								</el-form-item>
							</el-form>
						</el-card>
					</div>
...
export default {
	...
	methods: {
		getFlightPlan(){
			va.get("/flightPlan/" + this.planId)
					.then(res => {
						this.flightPlan = res.data
					})
		},
		updateFlightPlan(){
			va.put("/flightPlan/", this.flightPlan)
					.then(() => {
						this.$message.success("修改成功，返回可执行航线列表")
